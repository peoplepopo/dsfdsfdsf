...
const io = {
  ...
  moveInCircles: class extends IO {
    constructor(body) {
      super(body);
      this.acceptsFromTop = false;
  ...
  listenToPlayer: class extends IO {
    constructor(b, p) {
      super(b);
      this.player = p;
      this.acceptsFromTop = false;
  ...
  goToMasterTarget: class extends IO {
    constructor(body) {
      super(body);
      this.myGoal = {
  ...
  nearestDifferentMaster: class extends IO {
    constructor(body) {
      super(body);
  ...
  hangOutNearMaster: class extends IO {
    constructor(body) {
      super(body);
      this.acceptsFromTop = false;
      this.orbit = 30;
  ...
  fleeAtLowHealth: class extends IO {
    constructor(b) {
      super(b);
...
var bringToLife = (() => {
  ...
  return my => {
    ...
    if (TWO_ZEROS !== my.invisible) {
      ...
      my.alpha =
        x * x + y * y <= 0.01
          ? Math.max(0, my.alpha - my.invisible[0]) //invisible
          : Math.min(1, my.alpha + my.invisible[1]); //visible
    }
...
class Entity {
  ...
  define(set) {
    ...
    if (set.CONTROLLERS != null) {
      this.addController(set.CONTROLLERS.map(ioName => new io[ioName](this)));
    }
  ...
  contemplationOfMortality() {
    ...
    if (this.isDead()) {
      ...
      killers.forEach(instance => {
        ...
        if (this.type === "tank") {
          if (killers.size > 1) instance.killCount.assists++;
    ...
    connect: (() => {
      ...
      const spawn = (() => {
        ...
        return (socket, name) => {
          ...
          body.addController(new io.listenToPlayer(body, player)); // Make it listen
...
var maintainloop = (() => {
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      /*let npcs = entities.filter*/ entities.forEach(function npcCensus(
        instance
      ) {
...
// Turn the server on
{
  ...
  new (require("ws")).Server({ server: server }).on(
    "connection",
    sockets.connect
  );
}
